...
export default function TheResult({ ele }: { ele: CaseStudy }) {
  // Guard against unexpected data shape
  if (!ele.result || ele.result.length < 4) {
    // Optionally render a fallback; for now, render nothing to satisfy types
    return null;
  }

  const r0 = ele.result[0]!;
  const r1 = ele.result[1]!;
  const r2 = ele.result[2]!;
  const r3 = ele.result[3]!;
  return (
