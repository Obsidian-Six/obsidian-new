...
export default function TheImpact({ ele }: { ele: CaseStudy }) {
  return (
    <>
      <div className="w-[80%] mx-auto">
        ...
        {ele.impact.map((item: any, j: number) => (
          <div key={j}>
            <div className="grid md:grid-cols-2 md:gap-32 sm:gap-10 gap-6 items-center">
              <p className="md:text-6xl sm:text-4xl text-2xl  poppins font-semibold textmain leading-normal">
                {item.name}
              </p>
              <p className="md:text-2xl sm:text-xl text-lg inter font-normal textmain leading-normal">
                {item.detail}
              </p>
            </div>
      ...
      <div className="md:my-32 my-10">
        <Image
          ...
          width={320}
          height={176}
          className="md:h-44 h-20 mx-auto w-auto"
        />
      </div>
    </>
  );
