...
const HomeService = () => {
  ...
  const ServiceContent = ({
    ...
    <MotionDiv
      ...
      animate={
        isDesktop ? { opacity: 1, y: 0 } : { opacity: 1, height: "auto" }
      }
    ...
    >
      ...
      <div
        className={`grid grid-cols-1 md:grid-cols-[1.1fr_1.4fr] gap-4 items-start ${isDesktop ? "h-[40%] pt-4" : "mt-4 pb-4"}`}
      >
  ...
  return (
    <>
      <section
        id="services"
        className="max-w-7xl mx-auto my-12 lg:my-16 px-6 overflow-hidden h-fit lg:h-[75vh] lg:max-h-[650px] flex flex-col justify-between"
      >
          ...
          <div className="flex flex-col justify-start h-full">
            ...
            <div className="flex flex-col border-t border-t-[#19183A]/10 w-full">
              {services.map((service) => (
                <div
                  key={service.name}
                  className="border-b border-b-[#19183A]/10"
                >
                  <button
                    suppressHydrationWarning
                    type="button"
                    className={`w-full text-left flex items-center justify-between text-lg md:text-xl uppercase font-poppins py-4 transition-all duration-300 ${
                  ...
                  >
                    <span className="max-w-[85%] tracking-tight">
                      {service.name}
                    </span>
                    <BsArrowRightCircle
                      className={`text-lg md:text-xl transition-transform duration-300 ${
                        selectedService.name === service.name
                          ? "rotate-[-45deg] text-[#052D69]"
                          : "text-slate-400"
                      }`}
                    />
                  </button>
