...
export default function BlogPostClient({ slug }: { slug: string }) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-4 border-slate-100 border-t-[#052D69] rounded-full animate-spin"></div>
          <div className="text-[#052D69] font-bold">Loading Story...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white min-h-screen font-sans">
      {/* 1. HERO/HEADER SECTION - Changed bg-[#F3E8FF] (purple) to a light blue tint bg-[#E6F0F9] */}
      <section className="pt-32 md:pt-44 pb-12 md:pb-16 bg-[#E6F0F9]">
        <div className="max-w-7xl mx-auto px-4 md:px-6 flex flex-col lg:flex-row items-center gap-8 md:gap-12">
          <div className="flex-1 space-y-4 md:space-y-6 text-center lg:text-left">
            ...
            <div className="flex flex-wrap justify-center lg:justify-start items-center gap-4 md:gap-6 text-slate-600 text-sm md:text-base font-bold">
              <span className="text-[#052D69] italic underline decoration-[#052D69] underline-offset-4">By Obsidian Six</span>
              ...
              <span className="hidden sm:flex items-center gap-2 opacity-70">
                <span className="w-1.5 h-1.5 bg-[#052D69] rounded-full"></span>
                10 min read
              </span>
            </div>
          </div>
      ...
      <section className="py-12 md:py-20">
        <div className="max-w-7xl mx-auto px-4 md:px-6 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          <div className="lg:col-span-8 order-1">
            {post.content?.rendered ? (
              <div
                ...
                /* Links - Explicitly set to target blue */
                prose-a:text-[#052D69] prose-a:font-black prose-a:underline
          ...
          <aside className="lg:col-span-4 order-2">
            {/* 
          Added 'max-h-[calc(100vh-120px)]' to keep it within the viewport 
          Added 'overflow-y-auto' to enable scrolling
          Added custom scrollbar classes for a sleek blue look
      */}
            <div className="lg:sticky lg:top-28 bg-[#F9F9FB] p-6 md:p-4 rounded-3xl border border-slate-200 shadow-sm 
                      max-h-[calc(100vh-150px)] overflow-y-auto
                      scrollbar-thin scrollbar-thumb-[#052D69] scrollbar-track-slate-100">

              <h3 className="text-2xl md:text-3xl font-extrabold text-[#052D69] mb-4 md:mb-6 tracking-tight sticky top-0 bg-[#F9F9FB] pb-2 z-10">
      ...
      <section className="py-12 md:py-20 border-t border-slate-100 bg-slate-50/50">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="flex items-center justify-between mb-8 md:mb-12">
            ...
            <Link href="/blogs" className="text-[#052D69] font-black text-sm uppercase tracking-widest hover:underline">View All</Link>
          </div>
            ...
            {morePosts.map((p: any) => {
              ...
              return (
                <Link href={`/blogs/${p.slug}`} key={p.id} className="group space-y-5">
                  ...
                  <p
                    className="font-extrabold text-slate-900 text-xl md:text-2xl group-hover:text-[#052D69] transition-colors line-clamp-2 leading-tight tracking-tight"
