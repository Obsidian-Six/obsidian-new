...
//               <div className="text-xs text-[#052D69] flex items-center gap-2 uppercase">
//                 <p className="h-2.5 w-2.5 bg-[#052D69]" /> {caseStudy.name}
//               </div>
...
export default async function Page() {
  ...
  return (
    <div className="px-5 md:px-14">
      <p className="p-10 w-full bg-white"></p>
      {/* <div className="md:text-6xl sm:text-4xl text-3xl text-main font-light poppins my-10 w-[80%] mx-auto"> */}
      <div className="md:text-6xl sm:text-4xl text-3xl text-main font-light poppins my-10 w-[100%] mx-auto">
        CASE STUDIES
      </div>
      {/* <div className="grid md:grid-cols-3 sm:grid-cols-2 w-[80%] mx-auto gap-14 my-10"> */}
      <div className="grid md:grid-cols-2 sm:grid-cols-1 lg:grid-cols-3 w-[100%] mx-auto gap-x-10 gap-y-10 md:gap-x-6 md:gap-y-6 lg:gap-x-10 lg:gap-y-10 my-10 content-stretch">
        {combinedCaseStudies.map((caseStudy: CaseStudy, index: number) => (
          <MotionDiv
            key={index}
            // whileTap={{ scale: 0.85 }}
            className="text-main cursor-pointer grid content-stretch"
          >
            <Link
              href={`/case-studies/${getSlug(caseStudy.slug)}`}
              className="flex flex-col"
            >
              <div className="text-xs text-[#052D69] flex items-center gap-2 uppercase">
                <p className="h-2.5 w-2.5 bg-[#052D69]" /> {caseStudy.name}
              </div>
              <div className="my-1.5 text-xl max-md:text-lg">
                <p className="">{caseStudy.category}</p>
              </div>
              <div className="relative my-2.5 h-50 md:h-80 aspect-square overflow-hidden">
                <Image
                  // src={caseStudy.image}
                  // alt={caseStudy.name}
                  // width={800}
                  ...
                  src={caseStudy.image}
                  alt={caseStudy.name}
                  fill /* Parent ki exact height (h-52 / md:h-80) ko force karega */
                  className="object-cover" /* Sabhi images uniformly crop hongi aur bilkul barabar dikhengi */
